...
declare -r DOCKER_REGISTRY="https://hub.docker.com/repository/docker/ping2alexander/priv_hub"
declare -r DOCKER_SONIC_MGMT="docker-onl-auto"
declare -r LOCAL_IMAGE_NAME="docker-onl-auto-$(echo "${USER}" | tr '[:upper:]' '[:lower:]')"
declare -r LOCAL_IMAGE_TAG="master"
declare -r LOCAL_IMAGE="${LOCAL_IMAGE_NAME}:${LOCAL_IMAGE_TAG}"

declare -r ROOT_PASS="root"
declare -r USER_PASS="password"
...
function show_help_and_exit() {
    ...
    echo "                       If no value is provided, will use default value provided below:"
    echo "                         1. The local image named \"docker-onl-auto\""
    echo "  -d <directory>       specify directory inside container to bind mount to onl-auto root (default: \"/var/src\")"
    ...
    echo
    echo "Examples:"
    echo "  ./${SCRIPT_NAME} -n onl-auto-${USER}_master"
    echo "  ./${SCRIPT_NAME} -n onl-auto-${USER}_master -i docker-onl-auto"
    echo "  ./${SCRIPT_NAME} -n onl-auto-${USER}_master -d /var/src"
    echo "  ./${SCRIPT_NAME} -n onl-auto-${USER}_master -m /my/working/dir"
    echo "  ./${SCRIPT_NAME} -n onl-auto-${USER}_master -p 192.0.2.1:8080:80/tcp"
    echo "  ./${SCRIPT_NAME} -h"
    echo
    exit ${1}
}
...
function show_local_container_login() {
    ...
    echo "SSH:  ssh -i ~/.ssh/id_rsa_docker_onl_auto ${USER}@${CONTAINER_IPV4}"
    echo "******************************************************************************"
}
...
function setup_local_image() {
    ...
    PRIVKEY_FILE="${HOME}/.ssh/id_rsa_docker_onl_auto"
    PUBKEY_FILE="${HOME}/.ssh/id_rsa_docker_onl_auto.pub"
...
EOF
    ...
    log_error "1.failed to build docker image: ${LOCAL_IMAGE}"

    log_info "cleanup a temporary dir: ${TMP_DIR}"
    rm -rf "${TMP_DIR}"

    if ! docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "^${LOCAL_IMAGE}$"; then
        exit_failure "2.failed to build docker image: ${LOCAL_IMAGE}"
    fi
}
...
exit_success "onl-auto configuration is done!"
